...
import { CreditCard, Wallet, Copy, Check, QrCode } from 'lucide-react';
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    {config.registryItems.map((item, idx) => (
                        <ScrollReveal key={idx} variant="up" delay={idx * 100 + 200}>
                            <div className="bg-white p-8 rounded-2xl border border-[#E6D2B5]/30 shadow-sm hover:shadow-md transition-all duration-500 group relative overflow-hidden">
                                ...
                                <div className="flex flex-col items-center text-center relative z-10">
                                    <div className="bg-[#FAF9F6] p-4 rounded-full mb-6 text-[#B08D55] ring-1 ring-[#E6D2B5]/20">
                                        {item.type === 'bank' ? <CreditCard size={32} strokeWidth={1.5} /> : <Wallet size={32} strokeWidth={1.5} />}
                                    </div>
                                    ...
                                    <div className="w-full space-y-4 pt-4 border-t border-[#E6D2B5]/10">
