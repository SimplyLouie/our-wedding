...
// 3. Local Imports (Your new structure)
...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        // Default to anonymous for guests
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (!heroLoaded) setHeroLoaded(true); // Failsafe
    }, 5000);
  ...
  const handleSaveToDb = async () => {
    if (!user) return;
    setIsSaving(true);
    // Return promise for AdminPanel toast handling
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    // NOTE: Ensure you created this user in Firebase Console -> Auth
  ...
  const getGoogleCalendarUrl = () => {
    ...
    // Convert 2026-07-04T14:00 to 20260704T140000Z format
  ...
  return (
    <div className="bg-[#FAF9F6] min-h-screen text-[#43342E] font-sans selection:bg-[#C5A059] selection:text-white relative">
      <style>{`
        ...
        .confetti-0 { animation-delay: 0s; animation-duration: 2.5s; } .confetti-1 { animation-delay: 1s; animation-duration: 3s; } .confetti-2 { animation-delay: 0.5s; animation-duration: 2.8s; } .confetti-3 { animation-delay: 1.5s; animation-duration: 3.2s; }
        ...
        <div className="fixed inset-0 z-[70] bg-[#1F1815]/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white p-10 max-w-sm w-full text-center shadow-2xl border-t-4 border-[#B08D55] animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleLogin}>
              <input type="password" placeholder="Password" className={`w-full border-b p-3 text-center tracking-[0.2em] text-xl focus:outline-none mb-8 transition-colors ${loginError ? 'border-red-500 text-red-500 animate-shake placeholder-red-300' : 'border-[#E6D2B5] focus:border-[#43342E] text-[#43342E]'}`} value={adminPass} onChange={(e) => setAdminPass(e.target.value)} autoFocus />
      ...
      <section id="home" className="relative h-[100dvh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 flex flex-col justify-center h-full pt-10">
          ...
          <ScrollReveal variant="up" delay={600}>
            <div className="flex items-center justify-center gap-6 text-lg md:text-2xl font-light italic mb-8 md:mb-12 text-[#F2E8DC]">
          ...
          <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-float opacity-80 hover:opacity-100 transition-opacity z-20 cursor-pointer pt-10">
            <a href="#story" onClick={handleScrollClick} className="text-[#F9F4EF] hover:text-[#C5A059] transition-colors flex flex-col items-center gap-2 group p-4">
      ...
      <div className="h-[60vh] relative bg-fixed bg-center bg-cover flex items-center justify-center" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1511285560982-1356c11d4606?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80')" }}>
        ...
        <div className="relative z-10 text-center text-[#F9F4EF] px-6 max-w-5xl mx-auto py-16 backdrop-blur-[2px] border-y border-[#F9F4EF]/20">
          <ScrollReveal variant="zoom"><h2 className="font-serif text-4xl md:text-6xl italic font-light tracking-wide leading-tight">"And so the adventure begins"</h2></ScrollReveal>
        </div>
      </div>
            ...
            {config.events?.map((event, idx) => (
              <ScrollReveal key={idx} delay={200 * (idx + 1)} variant="up" className="h-full">
                <div className="bg-white/80 backdrop-blur-sm p-12 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] transition-all duration-500 text-center h-full group border-b-4 border-transparent hover:border-[#DBC1A7] hover:-translate-y-2 rounded-sm">
                  ...
                  <a href={event.mapLink || '#'} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center text-[#8C7C72] text-[10px] uppercase tracking-[0.2em] border border-[#E6D2B5] px-6 py-2 rounded-full hover:bg-[#E6D2B5] hover:text-white transition-all duration-300 gap-2 group/link">
        ...
        <div className="columns-2 md:columns-4 gap-4 px-4 max-w-[1600px] mx-auto space-y-4">
          {config.galleryImages?.map((src, i) => (
            <ScrollReveal key={i} delay={i * 50} variant="up">
              <div className="overflow-hidden group cursor-pointer relative bg-gray-100 rounded-sm break-inside-avoid shadow-sm hover:shadow-xl transition-all duration-500" onClick={() => setLightboxImg(src)}>
                <img src={src} loading="lazy" alt={`Gallery ${i}`} className="w-full h-auto object-cover transition-transform duration-[1.5s] ease-in-out group-hover:scale-110 filter grayscale-[10%] group-hover:grayscale-0" />
              ...
              !isSubmitted ? (
                <>
                  ...
                  <form onSubmit={handleRsvpSubmit} className="space-y-6 md:space-y-8">
                    <div className="group relative">
                      <input required type="text" name="name" className="w-full border-b border-[#DBC1A7] py-3 focus:outline-none focus:border-[#43342E] transition-colors bg-transparent text-[#43342E] placeholder-transparent peer" placeholder="Full Name" value={rsvpForm.name} onChange={(e) => setRsvpForm({ ...rsvpForm, name: e.target.value })} />
                    ...
                    <div className={`transition-all duration-500 overflow-hidden ${rsvpForm.attending === 'undecided' ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                      <div className="group relative mb-8">
                        <input type="email" name="email" className="w-full border-b border-[#DBC1A7] py-3 focus:outline-none focus:border-[#43342E] transition-colors bg-transparent text-[#43342E] placeholder-transparent peer" placeholder="Email Address" value={rsvpForm.email} required={rsvpForm.attending === 'undecided'} onChange={(e) => setRsvpForm({ ...rsvpForm, email: e.target.value })} />
                      ...
                      <div className="group relative">
                        <input type="date" name="followUpDate" className="w-full border-b border-[#DBC1A7] py-3 focus:outline-none focus:border-[#43342E] transition-colors bg-transparent text-[#43342E] placeholder-transparent peer" placeholder="Follow Up Date" value={rsvpForm.followUpDate} required={rsvpForm.attending === 'undecided'} onChange={(e) => setRsvpForm({ ...rsvpForm, followUpDate: e.target.value })} />
                    ...
                    <div className="flex flex-col md:flex-row gap-6 md:gap-8 pt-2">
                      <div className="w-full md:w-1/2">
                        ...
                        <select name="guests" className="w-full border-b border-[#DBC1A7] py-2 focus:outline-none focus:border-[#43342E] bg-transparent text-[#43342E]" value={rsvpForm.guests} onChange={(e) => setRsvpForm({ ...rsvpForm, guests: e.target.value })}>
                      ...
                      <div className="w-full md:w-1/2">
                        ...
                        <select name="attending" className="w-full border-b border-[#DBC1A7] py-2 focus:outline-none focus:border-[#43342E] bg-transparent text-[#43342E]" value={rsvpForm.attending} onChange={(e) => setRsvpForm({ ...rsvpForm, attending: e.target.value })}>
                    ...
                    <div className="group relative mt-6">
                      <textarea name="message" className="w-full border-b border-[#DBC1A7] py-3 focus:outline-none focus:border-[#43342E] transition-colors bg-transparent text-[#43342E] placeholder-transparent peer resize-none" placeholder="Message to the Couple" rows="2" value={rsvpForm.message} onChange={(e) => setRsvpForm({ ...rsvpForm, message: e.target.value })} />
                    ...
                    <button type="submit" disabled={isSubmitting} className="w-full bg-[#43342E] text-[#F9F4EF] py-4 mt-8 text-xs font-bold tracking-[0.25em] hover:bg-[#5D4B42] transition-all duration-300 shadow-md hover:shadow-lg uppercase disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center group">
                ...
                <div className="h-full flex flex-col items-center justify-center text-center animate-fade-in py-10 relative overflow-hidden">
                  ...
                  {rsvpForm.attending === 'yes' && (
                    <a href={getGoogleCalendarUrl()} target="_blank" rel="noopener noreferrer" className="relative z-10 flex items-center gap-2 text-[#43342E] border border-[#43342E] px-6 py-3 rounded-full uppercase text-[10px] font-bold tracking-widest hover:bg-[#43342E] hover:text-white transition-all mb-4">
                      <CalendarPlus size={14} /> Add to Calendar
                    </a>
                  )}

                  <button onClick={() => setIsSubmitted(false)} className="mt-4 text-[#B08D55] text-[10px] uppercase tracking-[0.2em] border-b border-[#B08D55] pb-1 hover:text-[#43342E] hover:border-[#43342E] transition-colors relative z-10">Submit another response</button>
                </div>
              )
            )}
          </div>
      ...
      <footer className="bg-[#1F1815] text-[#8C7C72] py-20 text-center border-t-8 border-[#2E2622]">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex flex-col items-center justify-center gap-6 border-t border-[#43342E]/50 pt-10 mt-6 max-w-xs mx-auto">
            ...
            <button onClick={() => setShowAdminLogin(true)} className="text-[9px] text-[#43342E] hover:text-[#B08D55] transition-colors uppercase tracking-widest font-bold flex items-center gap-1 opacity-50 hover:opacity-100"><Lock size={10} /> Admin Access</button>
          </div>
        </div>
      </footer>
    </div>
